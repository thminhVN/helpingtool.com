...
{
    public $entityClass = '\Entity\StPage';
    public $controller = 'page';
    public $form = '\Admin\Form\PageForm';
    ...
    public function pagingAction()
    {
        ...
        if($count > 0)
        {
            ...
            foreach($data as $key => $object)
            {
                ...
                $row['seo_description'] = $object['seo_description'];
                $row['option_status'] = $config->getConfig('status');
                $row['option_priority'] = $config->getConfig('priority');
                $output['aaData'][] = $row;
            }   
        }
